...
Send it when you create a verification for a record that exists in your system. ezyshield stores it on the verification so you can retrieve related verification history later.
...
`attribution_id` is especially useful with contact identity flows. It lets successive verifications for the same record remain connected, and it gives ezyshield context for repeat KYC or biometric flows where previous successful identity evidence may reduce friction.
